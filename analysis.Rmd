...
First, let's process the storm data dates and times into R POSIX time.
...
```{r}
  ...
  computeMagnitude <- function(damage, damageExp) {
    # Determine if damageExp is an allowed character or integer
    if (is.na(as.numeric(damageExp)) & damageExp %in% names(expLookup)) {
      damageExp <- expLookup[[damageExp]]
    } else {  # just return the raw damage
      damageExp <- 0
    }
    
    return(damage * 10 ^ damageExp)
  }
```
